...
- `control_plane_runtime_foundation_smoke.py`
  - 验证 v2.1.0 RuntimeContract foundation、registered task type validator contract、projection candidate-only boundary、deterministic validation run 和 runtime persistence readout；传入 `--web-url` 时额外执行 System Validation DOM smoke。
