...
func buildOpenAPISchemas() map[string]any {
	schemas := map[string]any{
		...
		"AgentRunToolDeclaration": objectSchema(map[string]any{
			"type":        stringSchema("tool 类型；OpenAI-compatible function tool 可使用 function。", "function"),
			"name":        stringSchema("简写 tool 名称；function.name 为空时使用。", "web_search"),
			"description": stringSchema("tool 描述。", "Search public market information."),
		...
		"AgentRunCreateRequest": objectSchema(map[string]any{
			...
			"tools":                    arraySchema(refSchema("AgentRunToolDeclaration")),
			"tool_choice":              map[string]any{"description": "预留给 OpenAI-compatible tool_choice 的声明态输入。"},
		...
		"AgentRunResponse": objectSchema(map[string]any{
			...
			"tool_calls":          arraySchema(map[string]any{"type": "object", "additionalProperties": true}),
