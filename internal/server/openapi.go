...
func buildOpenAPIPaths() map[string]any {
	paths := map[string]any{
		...
		"/api/agent/runs/{runID}": map[string]any{
			"get": map[string]any{
				"tags":        []string{"agent-runs"},
		...
		"/api/agent/runs/{runID}/resume": map[string]any{
			"post": map[string]any{
				"tags":        []string{"agent-runs"},
		...
		"/api/agent/runs/{runID}/cancel": map[string]any{
			"post": map[string]any{
				"tags":        []string{"agent-runs"},
		...
		"/api/agent/runs/{runID}/trace": map[string]any{
			"get": map[string]any{
				"tags":        []string{"agent-runs"},
		...
		"/api/memory/write": map[string]any{
			"post": map[string]any{
				"tags":        []string{"memory"},
				"summary":     "写入应用拥有的作用域记忆摘要",
				"operationId": "writeExternalMemory",
				"requestBody": jsonRequest("ExternalMemoryWriteRequest", true),
				"responses": map[string]any{
					"201": jsonResponse("记忆写入结果", "ExternalMemoryWriteResponse"),
					"400": jsonResponse("错误请求", "ErrorResponse"),
				},
			},
		},
		"/api/memory/query": map[string]any{
			"post": map[string]any{
				"tags":        []string{"memory"},
				"summary":     "按应用、所有者和作用域查询记忆摘要",
				"operationId": "queryExternalMemory",
				"requestBody": jsonRequest("ExternalMemoryQueryRequest", true),
				"responses": map[string]any{
					"200": jsonResponse("记忆查询结果", "ExternalMemoryQueryResponse"),
					"400": jsonResponse("错误请求", "ErrorResponse"),
				},
			},
		},
		"/api/context-assets/resolve": map[string]any{
			"post": map[string]any{
				"tags":        []string{"context-assets"},
				"summary":     "将应用记忆摘要解析为只读上下文资产",
				"operationId": "resolveExternalContextAssets",
				"requestBody": jsonRequest("ExternalMemoryQueryRequest", true),
				"responses": map[string]any{
					"200": jsonResponse("上下文资产解析结果", "ExternalContextResolveResponse"),
					"400": jsonResponse("错误请求", "ErrorResponse"),
				},
			},
		},
		"/api/context-assets/assemble": map[string]any{
			"post": map[string]any{
				"tags":        []string{"context-assets"},
				"summary":     "组装应用记忆与上下文资产的有效视图",
				"operationId": "assembleExternalContextAssets",
				"requestBody": jsonRequest("ExternalContextAssembleRequest", true),
				"responses": map[string]any{
					"200": jsonResponse("上下文资产组装结果", "ExternalContextAssembleResponse"),
					"400": jsonResponse("错误请求", "ErrorResponse"),
				},
			},
		},
		...
		"/api/control-plane/runtime/runs/{runID}/trace-payloads/{payloadRef}": map[string]any{
			"get": map[string]any{
				"tags":        []string{"control-plane"},
				"summary":     "按需审计读取一条加密 trace 明细",
				"operationId": "getControlPlanePrivilegedTracePayload",
				"description": "仅接受有效 Control Plane session。响应不会缓存，每次成功、拒绝、未找到、过期或失败读取都会持久化审计；普通 Agent API 不暴露 payload ref。",
				"parameters": append(pathIDParameter("runID", "runtime run ID"), map[string]any{
					"name": "payloadRef", "in": "path", "required": true,
					"schema": map[string]any{"type": "string"}, "description": "Opaque privileged payload reference.",
				}),
				"responses": map[string]any{
					"200": jsonResponse("decrypted and redacted trace payload", "PrivilegedTracePayloadResponse"),
					"401": jsonResponse("control-plane authentication required", "ControlPlaneAuthStatus"),
					"403": jsonResponse("privileged trace reads disabled", "ErrorResponse"),
					"404": jsonResponse("run or payload mismatch", "ErrorResponse"),
					"410": jsonResponse("payload expired", "ErrorResponse"),
					"503": jsonResponse("durable audit unavailable", "ErrorResponse"),
				},
			},
		},
		...
		"/api/control-plane/runtime/system-truth/lifecycle": map[string]any{
			"get": map[string]any{
				"tags":        []string{"control-plane"},
				"summary":     "读取 System Truth 生命周期记录",
				"operationId": "listControlPlaneRuntimeSystemTruthLifecycle",
				"description": "读取 source、draft、compile result 和 active pointer history，用于验证 source -> draft -> compile -> active -> rollback 写入链路。",
				"parameters": []map[string]any{
					queryStringParameter("asset_id", "可选 system truth asset ID 过滤条件。"),
					queryStringParameter("source_id", "可选 source ID 过滤条件。"),
					queryStringParameter("draft_id", "可选 draft ID 过滤条件。"),
					queryStringParameter("status", "可选生命周期状态过滤条件。"),
					queryIntParameter("limit", "返回数量上限。"),
				},
				"responses": map[string]any{
					"200": jsonResponse("system truth lifecycle readout", "SystemTruthLifecycleReadoutResponse"),
					"503": jsonResponse("runtime persistence store unavailable", "ErrorResponse"),
				},
			},
		},
		"/api/control-plane/runtime/system-truth/sources": map[string]any{
			"post": map[string]any{
				"tags":        []string{"control-plane"},
				"summary":     "追加一个 System Truth source",
				"operationId": "createControlPlaneRuntimeSystemTruthSource",
				"requestBody": jsonRequest("SystemTruthSourceCreateRequest", true),
				"responses": map[string]any{
					"201": jsonResponse("system truth source", "SystemTruthSource"),
					"400": jsonResponse("invalid system truth source payload", "ErrorResponse"),
					"503": jsonResponse("runtime persistence store unavailable", "ErrorResponse"),
				},
			},
		},
		"/api/control-plane/runtime/system-truth/drafts": map[string]any{
			"post": map[string]any{
				"tags":        []string{"control-plane"},
				"summary":     "追加一个 System Truth draft",
				"operationId": "createControlPlaneRuntimeSystemTruthDraft",
				"requestBody": jsonRequest("SystemTruthDraftCreateRequest", true),
				"responses": map[string]any{
					"201": jsonResponse("system truth draft", "SystemTruthDraft"),
					"400": jsonResponse("invalid system truth draft payload", "ErrorResponse"),
					"503": jsonResponse("runtime persistence store unavailable", "ErrorResponse"),
				},
			},
		},
		"/api/control-plane/runtime/system-truth/drafts/{draftID}/compile": map[string]any{
			"post": map[string]any{
				"tags":        []string{"control-plane"},
				"summary":     "追加一个 System Truth compile result",
				"operationId": "compileControlPlaneRuntimeSystemTruthDraft",
				"parameters":  pathIDParameter("draftID", "system truth draft ID"),
				"requestBody": jsonRequest("SystemTruthCompileRequest", false),
				"responses": map[string]any{
					"201": jsonResponse("system truth compile result", "SystemTruthCompileResult"),
					"400": jsonResponse("invalid system truth compile payload", "ErrorResponse"),
					"503": jsonResponse("runtime persistence store unavailable", "ErrorResponse"),
				},
			},
		},
		"/api/control-plane/runtime/system-truth/compile-results/{compileID}/activate": map[string]any{
			"post": map[string]any{
				"tags":        []string{"control-plane"},
				"summary":     "激活一个成功的 System Truth compile result",
				"operationId": "activateControlPlaneRuntimeSystemTruthCompileResult",
				"parameters":  pathIDParameter("compileID", "system truth compile result ID"),
				"requestBody": jsonRequest("SystemTruthActivateRequest", false),
				"responses": map[string]any{
					"201": jsonResponse("system truth active version", "SystemTruthActiveVersion"),
					"400": jsonResponse("invalid system truth activation payload", "ErrorResponse"),
					"503": jsonResponse("runtime persistence store unavailable", "ErrorResponse"),
				},
			},
		},
		"/api/control-plane/runtime/system-truth/active-versions/{activeID}/rollback": map[string]any{
			"post": map[string]any{
				"tags":        []string{"control-plane"},
				"summary":     "回滚到一个历史 System Truth active version",
				"operationId": "rollbackControlPlaneRuntimeSystemTruthActiveVersion",
				"parameters":  pathIDParameter("activeID", "system truth active version ID"),
				"requestBody": jsonRequest("SystemTruthActivateRequest", false),
				"responses": map[string]any{
					"201": jsonResponse("system truth active version", "SystemTruthActiveVersion"),
					"400": jsonResponse("invalid system truth rollback payload", "ErrorResponse"),
					"503": jsonResponse("runtime persistence store unavailable", "ErrorResponse"),
				},
			},
		},
...
func buildOpenAPISchemas() map[string]any {
	schemas := map[string]any{
		...
		"ExternalMemoryWriteRequest": objectSchema(map[string]any{
			"app_id":         stringSchema("Owning application identifier.", "fund-assistant"),
			"owner_id":       stringSchema("Application-owned subject identifier.", "user-1"),
			"scope":          stringSchema("Application-defined memory scope.", "investor-profile"),
			"kind":           stringSchema("Generic summary kind.", "profile-summary"),
			"summary":        stringSchema("Safe summary text, not a business object payload.", "Balanced risk profile."),
			"schema_version": stringSchema("Optional app summary schema version.", "fund-profile.v1"),
			"metadata":       map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "string"}},
		}, []string{"app_id", "owner_id", "scope", "kind", "summary"}),
		"ExternalMemoryQueryRequest": objectSchema(map[string]any{
			"app_id":   stringSchema("Owning application identifier.", "fund-assistant"),
			"owner_id": stringSchema("Application-owned subject identifier.", "user-1"),
			"scope":    stringSchema("Application-defined memory scope.", "investor-profile"),
			"limit":    map[string]any{"type": "integer", "format": "int32", "minimum": 1, "maximum": 50},
		}, []string{"app_id", "owner_id", "scope"}),
		"ExternalMemoryRecord": objectSchema(map[string]any{
		...
		}, []string{"id", "app_id", "owner_id", "scope", "kind", "summary", "schema_version"}),
		"ExternalMemoryTrace": objectSchema(map[string]any{
			"operation":  map[string]any{"type": "string", "enum": []string{"memory_write", "memory_query"}},
			"app_id":     map[string]any{"type": "string"},
			"owner_id":   map[string]any{"type": "string"},
			"scope":      map[string]any{"type": "string"},
			"record_ids": arraySchema(map[string]any{"type": "string"}),
			"created_at": map[string]any{"type": "string", "format": "date-time"},
		}, []string{"operation", "app_id", "owner_id", "scope", "record_ids", "created_at"}),
		"ExternalMemoryWriteResponse":    objectSchema(map[string]any{"item": refSchema("ExternalMemoryRecord"), "trace": refSchema("ExternalMemoryTrace")}, []string{"item", "trace"}),
		"ExternalMemoryQueryResponse":    objectSchema(map[string]any{"items": arraySchema(refSchema("ExternalMemoryRecord")), "trace": refSchema("ExternalMemoryTrace")}, []string{"items", "trace"}),
		"ExternalContextResolveResponse": objectSchema(map[string]any{"assets": arraySchema(map[string]any{"type": "object", "additionalProperties": true}), "memory_trace": refSchema("ExternalMemoryTrace")}, []string{"assets", "memory_trace"}),
		"ExternalContextAssembleRequest": objectSchema(map[string]any{
			"app_id":              stringSchema("Owning application identifier.", "fund-assistant"),
			"owner_id":            stringSchema("Application-owned subject identifier.", "user-1"),
			"scope":               stringSchema("Application-defined memory scope.", "investor-profile"),
			"limit":               map[string]any{"type": "integer", "format": "int32", "minimum": 1, "maximum": 50},
			...
			"assets":              arraySchema(map[string]any{"type": "object", "additionalProperties": true}),
		}, []string{"app_id", "owner_id", "scope"}),
		"ExternalContextAssembleResponse": objectSchema(map[string]any{
			"assets":              arraySchema(map[string]any{"type": "object", "additionalProperties": true}),
			"memory_trace":        refSchema("ExternalMemoryTrace"),
			"context_trace":       map[string]any{"type": "object", "additionalProperties": true},
			"effective_views":     map[string]any{"type": "object", "additionalProperties": true},
			"context_compression": map[string]any{"type": "object", "additionalProperties": true},
		}, []string{"assets", "memory_trace", "context_trace", "effective_views", "context_compression"}),
		...
		"AgentRunToolFunction": objectSchema(map[string]any{
			"name":        stringSchema("OpenAI-compatible function name。", "web_search"),
			"description": stringSchema("OpenAI-compatible function description。", "Search public market information."),
			"parameters":  map[string]any{"type": "object", "description": "Root object JSON Schema for function arguments.", "additionalProperties": true},
		}, []string{"name"}),
		"AgentRunToolDeclaration": objectSchema(map[string]any{
			"type":     map[string]any{"type": "string", "enum": []string{"function"}},
			"function": refSchema("AgentRunToolFunction"),
			"metadata": map[string]any{"type": "object", "additionalProperties": true},
		}, []string{"type", "function"}),
		"AgentRunToolChoiceFunction": objectSchema(map[string]any{
			"type": map[string]any{"type": "string", "enum": []string{"function"}},
			"function": objectSchema(map[string]any{
				"name": stringSchema("Required function name.", "web_search"),
			}, []string{"name"}),
		}, []string{"type", "function"}),
		"AgentRunToolChoice": map[string]any{
			"oneOf": []any{
				map[string]any{"type": "string", "enum": []string{"none", "auto", "required"}},
				refSchema("AgentRunToolChoiceFunction"),
			},
		},
		"AgentRunFunctionCall": objectSchema(map[string]any{
			"name":      stringSchema("Function name.", "web_search"),
			"arguments": stringSchema("JSON-encoded function arguments.", `{"query":"S&P 500"}`),
		}, []string{"name", "arguments"}),
		"AgentRunToolCall": objectSchema(map[string]any{
			"index":    map[string]any{"type": "integer", "format": "int32"},
			"id":       stringSchema("Stable tool call ID.", "call_019d7286"),
			"type":     map[string]any{"type": "string", "enum": []string{"function"}},
			"function": refSchema("AgentRunFunctionCall"),
			"status":   map[string]any{"type": "string", "enum": []string{"pending", "running", "completed", "failed"}},
		}, []string{"index", "id", "type", "function"}),
		"AgentRunToolResult": objectSchema(map[string]any{
			"role":         map[string]any{"type": "string", "enum": []string{"tool"}},
			"tool_call_id": stringSchema("Originating stable tool call ID.", "call_019d7286"),
			"name":         stringSchema("Function name.", "web_search"),
			"content":      stringSchema("Tool result content.", `{"items":[]}`),
			"status":       map[string]any{"type": "string", "enum": []string{"completed", "failed"}},
			"is_error":     map[string]any{"type": "boolean"},
			"error":        map[string]any{"type": "string"},
		}, []string{"role", "tool_call_id"}),
		"AgentRunMessage": objectSchema(map[string]any{
			"role":         map[string]any{"type": "string", "enum": []string{"assistant", "tool"}},
			"content":      map[string]any{"type": "string"},
			"tool_calls":   arraySchema(refSchema("AgentRunToolCall")),
		...
		"AgentRunCreateRequest": objectSchema(map[string]any{
			"goal":             stringSchema("目标驱动 run 的自然语言目标。", "Summarize portfolio risk and propose next actions."),
			...
			"tools": map[string]any{
				...
				"items": map[string]any{
					"oneOf": []any{
						...
						refSchema("AgentRunToolDeclaration"),
					},
				},
			},
			"tool_choice":              refSchema("AgentRunToolChoice"),
			"memory_scope":             map[string]any{"type": "object", "additionalProperties": true},
			"governance_refs":          arraySchema(map[string]any{"type": "string"}),
			"task_type":                stringSchema("可选 runtime task type；省略或 agent_run 时映射为已注册 chat。", "agent_run"),
			"task_subtype":             stringSchema("可选任务子类型。", "goal_directed"),
			"scene":                    stringSchema("可选场景标签。", "agent_runtime"),
			"session_id":               stringSchema("已有 session ID；省略时创建新 session。", "sess_019d7286"),
			"main_session_id":          stringSchema("主会话 ID。", "sess_main"),
			"workspace_id":             stringSchema("workspace ID。", "workspace-default"),
			"app_instance_id":          stringSchema("业务应用实例 ID。", "fund-assistant"),
			"app_session_id":           stringSchema("业务应用自己的 session ID。", "fund-session-1"),
			"integration_instance_id":  stringSchema("外部集成实例 ID。", "integration-1"),
			"workflow_run_id":          stringSchema("外部 workflow run ID。", "workflow-1"),
			"step_id":                  stringSchema("外部 workflow step ID。", "step-1"),
			"trigger_type":             stringSchema("触发类型。", "user"),
			"automation_task_id":       stringSchema("自动化任务 ID。", "automation-1"),
			"user_language":            stringSchema("用户语言偏好。", "zh-CN"),
			"desired_output_mode":      stringSchema("期望输出模式。", "text"),
			"global_context":           map[string]any{"type": "object", "additionalProperties": true},
			"app_context":              map[string]any{"type": "object", "additionalProperties": true},
			"input_payload":            map[string]any{"type": "object", "additionalProperties": true},
			"model_id":                 stringSchema("模型治理记录 ID。", "model_record_default"),
			"enabled_skills":           arraySchema(map[string]any{"type": "string"}),
			"enabled_tools":            arraySchema(map[string]any{"type": "string"}),
			"prompt_template":          stringSchema("请求级 prompt override。", "Answer in concise bullets."),
			"disabled_asset_types":     arraySchema(map[string]any{"type": "string"}),
			"asset_priority_overrides": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "integer", "format": "int32"}},
			"supplement":               refSchema("SupplementPayload"),
			"supplement_outcome":       map[string]any{"type": "string"},
			"resume_token":             stringSchema("等待态恢复 token；token 原文只来自调用方输入。", "resume-token"),
			"timeout_after_seconds":    map[string]any{"type": "integer", "format": "int32"},
			"disable_fast_path":        map[string]any{"type": "boolean"},
		}, nil),
		"AgentRunResumeRequest": objectSchema(map[string]any{
			"goal":                     stringSchema("续跑目标；省略时由 supplement 驱动。", "Continue with supplied facts."),
			"query":                    stringSchema("兼容字段；goal 为空时作为目标。", "Continue."),
			"supplement":               refSchema("SupplementPayload"),
			"supplement_outcome":       map[string]any{"type": "string"},
			"resume_token":             stringSchema("等待态恢复 token。", "resume-token"),
			"session_id":               stringSchema("原 session ID；补数恢复通常需要传入。", "sess_019d7286"),
			"main_session_id":          stringSchema("主会话 ID。", "sess_main"),
			"model_id":                 stringSchema("模型治理记录 ID。", "model_record_default"),
			"enabled_skills":           arraySchema(map[string]any{"type": "string"}),
			"enabled_tools":            arraySchema(map[string]any{"type": "string"}),
			...
			"timeout_after_seconds":    map[string]any{"type": "integer", "format": "int32"},
			"disable_fast_path":        map[string]any{"type": "boolean"},
			"prompt_template":          stringSchema("请求级 prompt override。", "Continue in concise bullets."),
			"disabled_asset_types":     arraySchema(map[string]any{"type": "string"}),
			"asset_priority_overrides": map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "integer", "format": "int32"}},
		}, nil),
		"AgentRunCancelRequest": objectSchema(map[string]any{
			"reason": stringSchema("调用方请求取消的原因。", "user_requested"),
		}, nil),
		"AgentRunTraceSummary": objectSchema(map[string]any{
			"run_id":             stringSchema("runtime run ID。", "run_20260507_001"),
			"step_count":         intSchema("step 数量。", 2),
			"event_count":        intSchema("lifecycle event 数量。", 6),
			"trace_count":        intSchema("trace 数量。", 3),
			"usage_count":        intSchema("usage 数量。", 1),
			"projection_count":   intSchema("candidate projection 数量。", 2),
			"checkpoint_count":   intSchema("checkpoint readout 数量。", 1),
			"last_event_type":    stringSchema("最后一个 lifecycle event 类型。", "run_terminal_observed"),
			"last_event_reason":  stringSchema("最后一个 lifecycle event 原因。", "runner_terminal_outcome_observed"),
			"last_trace_type":    stringSchema("最后一个 trace 类型。", "terminal_output_summary"),
			"last_trace_summary": stringSchema("最后一个 trace 摘要。", "assistant response candidate"),
		}, []string{"step_count", "event_count", "trace_count", "usage_count", "projection_count", "checkpoint_count"}),
		"AgentRunResponse": objectSchema(map[string]any{
			"request_id":          stringSchema("请求 ID。", "req-1783527999639258000-5474932f"),
			"run_id":              stringSchema("runtime run ID。", "run_20260507_001"),
			...
			"error":               stringSchema("错误摘要。", "requested model was not found"),
			"error_detail":        map[string]any{"type": "object", "additionalProperties": true},
			"run":                 refSchema("RuntimeRun"),
			"trace_summary":       refSchema("AgentRunTraceSummary"),
			"checkpoints":         arraySchema(refSchema("RuntimeCheckpointReadout")),
			"messages":            arraySchema(refSchema("AgentRunMessage")),
			"tool_calls":          arraySchema(refSchema("AgentRunToolCall")),
			"tool_results":        arraySchema(refSchema("AgentRunToolResult")),
		...
		"AgentRunTraceResponse": objectSchema(map[string]any{
			...
			"traces":      arraySchema(refSchema("RuntimeTrace")),
			"usage":       arraySchema(refSchema("RuntimeUsage")),
			"projections": arraySchema(refSchema("RuntimeProjectionCandidate")),
			"checkpoints": arraySchema(refSchema("RuntimeCheckpointReadout")),
		...
		"AgentRunTimelineItem": objectSchema(map[string]any{
			...
			"payload_ref":                map[string]any{"type": "string", "description": "Opaque reference exposed only by the Control Plane timeline."},
			"payload_status":             map[string]any{"type": "string", "enum": []string{"recorded", "disabled", "sampled_out", "size_exceeded", "redacted_empty", "expired", "capture_failed"}},
			"payload_unavailable_reason": map[string]any{"type": "string"},
		}, []string{"id", "kind", "timestamp", "source", "summary"}),
		"PrivilegedTracePayloadResponse": objectSchema(map[string]any{
			"payload_ref":          map[string]any{"type": "string"},
			"run_id":               map[string]any{"type": "string"},
			"step_id":              map[string]any{"type": "string"},
			"trace_type":           map[string]any{"type": "string"},
			"source":               map[string]any{"type": "string"},
			"schema_version":       map[string]any{"type": "string"},
			"payload":              map[string]any{"type": "object", "additionalProperties": true, "description": "Decrypted content after mandatory denylist redaction."},
			"payload_size":         map[string]any{"type": "integer", "format": "int32"},
			"redacted_field_count": map[string]any{"type": "integer", "format": "int32"},
			"created_at":           map[string]any{"type": "string", "format": "date-time"},
			"expires_at":           map[string]any{"type": "string", "format": "date-time"},
		}, []string{"payload_ref", "run_id", "trace_type", "source", "schema_version", "payload", "payload_size", "redacted_field_count", "created_at", "expires_at"}),
		...
		"SystemTruthSource": objectSchema(map[string]any{
			"id":           stringSchema("source 记录 ID。", "source_persona_default_001"),
			"asset_id":     stringSchema("system truth asset ID。", "persona.default"),
			"source_kind":  stringSchema("来源类型。", "operator_input"),
			"source_ref":   stringSchema("来源引用。", "system-validation"),
			"status":       stringSchema("source 状态。", "imported"),
			"content":      map[string]any{"type": "object", "additionalProperties": true},
			"content_hash": stringSchema("source 内容哈希。", "sha256:..."),
			"metadata":     map[string]any{"type": "object", "additionalProperties": true},
			"created_at":   dateTimeSchema("创建时间。", "2026-05-19T10:00:00Z"),
		}, []string{"id", "asset_id", "source_kind", "status", "created_at"}),
		"SystemTruthSourceCreateRequest": objectSchema(map[string]any{
			"asset_id":     stringSchema("system truth asset ID。", "persona.default"),
			"source_kind":  stringSchema("来源类型。", "operator_input"),
			"source_ref":   stringSchema("来源引用。", "system-validation"),
			"status":       stringSchema("source 状态；省略时为 imported。", "imported"),
			"content":      map[string]any{"type": "object", "additionalProperties": true},
			"content_hash": stringSchema("可选内容哈希；省略时后端按 content 生成。", "sha256:..."),
			"metadata":     map[string]any{"type": "object", "additionalProperties": true},
		}, []string{"asset_id", "source_kind"}),
		"SystemTruthDraft": objectSchema(map[string]any{
			"id":             stringSchema("draft 记录 ID。", "draft_persona_default_001"),
			"source_id":      stringSchema("source 记录 ID。", "source_persona_default_001"),
			"asset_id":       stringSchema("system truth asset ID。", "persona.default"),
			"status":         stringSchema("draft 状态。", "draft"),
			"author":         stringSchema("编辑人或来源。", "operator"),
			"reason":         stringSchema("编辑原因。", "adjust runtime guidance"),
			"base_active_id": stringSchema("编辑基于的 active version ID。", "active_persona_default_001"),
			"content":        map[string]any{"type": "object", "additionalProperties": true},
			"diff_summary":   stringSchema("差异摘要。", "update summary"),
			"metadata":       map[string]any{"type": "object", "additionalProperties": true},
			"created_at":     dateTimeSchema("创建时间。", "2026-05-19T10:00:00Z"),
			"updated_at":     dateTimeSchema("最近更新时间。", "2026-05-19T10:00:01Z"),
		}, []string{"id", "source_id", "asset_id", "status", "created_at", "updated_at"}),
		"SystemTruthDraftCreateRequest": objectSchema(map[string]any{
			"source_id":      stringSchema("source 记录 ID。", "source_persona_default_001"),
			"asset_id":       stringSchema("可选 system truth asset ID；省略时继承 source。", "persona.default"),
			"status":         stringSchema("draft 状态；省略时为 draft。", "draft"),
			"author":         stringSchema("编辑人或来源。", "operator"),
			"reason":         stringSchema("编辑原因。", "adjust runtime guidance"),
			"base_active_id": stringSchema("编辑基于的 active version ID。", "active_persona_default_001"),
			"content":        map[string]any{"type": "object", "additionalProperties": true},
		...
		"SystemTruthCompileResult": objectSchema(map[string]any{
			"id":               stringSchema("compile result ID。", "compile_persona_default_001"),
			"draft_id":         stringSchema("draft 记录 ID。", "draft_persona_default_001"),
			"asset_id":         stringSchema("system truth asset ID。", "persona.default"),
			"status":           stringSchema("compile 状态。", "succeeded"),
			"summary":          stringSchema("compile 摘要。", "compiled system truth draft"),
			"diagnostics":      map[string]any{"type": "object", "additionalProperties": true},
			"compiled_payload": map[string]any{"type": "object", "additionalProperties": true},
			"content_hash":     stringSchema("compiled payload 哈希。", "sha256:..."),
			"metadata":         map[string]any{"type": "object", "additionalProperties": true},
			"created_at":       dateTimeSchema("创建时间。", "2026-05-19T10:00:00Z"),
		}, []string{"id", "draft_id", "asset_id", "status", "created_at"}),
		"SystemTruthCompileRequest": objectSchema(map[string]any{
			"asset_id":         stringSchema("可选 system truth asset ID；省略时继承 draft。", "persona.default"),
			"status":           stringSchema("compile 状态；省略时为 succeeded。", "succeeded"),
			"summary":          stringSchema("compile 摘要。", "compiled system truth draft"),
			"diagnostics":      map[string]any{"type": "object", "additionalProperties": true},
			"compiled_payload": map[string]any{"type": "object", "additionalProperties": true},
			"content_hash":     stringSchema("可选 compiled payload 哈希；省略时后端生成。", "sha256:..."),
			"metadata":         map[string]any{"type": "object", "additionalProperties": true},
		}, nil),
		...
		"SystemTruthActivateRequest": objectSchema(map[string]any{
			"asset_id":         stringSchema("可选 system truth asset ID；省略时继承 compile result。", "persona.default"),
			"draft_id":         stringSchema("可选 draft ID；省略时继承 compile result。", "draft_persona_default_001"),
			"activated_by":     stringSchema("激活人或来源。", "operator"),
			"reason":           stringSchema("激活或回滚原因。", "activate compiled truth"),
			"rollback_from_id": stringSchema("可选回滚来源 active ID；正常激活通常为空。", "active_previous"),
			"metadata":         map[string]any{"type": "object", "additionalProperties": true},
		}, nil),
		"SystemTruthLifecycleReadoutResponse": objectSchema(map[string]any{
			"sources":         arraySchema(refSchema("SystemTruthSource")),
			"drafts":          arraySchema(refSchema("SystemTruthDraft")),
			"compile_results": arraySchema(refSchema("SystemTruthCompileResult")),
			"active_versions": arraySchema(refSchema("SystemTruthActiveVersion")),
		}, []string{"sources", "drafts", "compile_results", "active_versions"}),
		"RuntimeContractFoundationResponse": objectSchema(map[string]any{
			...
			"system_truth_sources":         arraySchema(refSchema("SystemTruthSource")),
			"system_truth_drafts":          arraySchema(refSchema("SystemTruthDraft")),
			"system_truth_compile_results": arraySchema(refSchema("SystemTruthCompileResult")),
